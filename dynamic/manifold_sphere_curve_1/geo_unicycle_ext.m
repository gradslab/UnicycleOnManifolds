...
h2 = atan2(x__2, x__1);
dh2 = [-x__2/(x__1^2+x__2^2);...
           x__1/(x__1^2+x__2^2);...
           0];
ddh2 = [2 * x__1 * x__2 / (x__1 ^ 2 + x__2 ^ 2) ^ 2 (-x__1 ^ 2 + x__2 ^ 2) / (x__1 ^ 2 + x__2 ^ 2) ^ 2 0; (-x__1 ^ 2 + x__2 ^ 2) / (x__1 ^ 2 + x__2 ^ 2) ^ 2 -2 * x__1 * x__2 / (x__1 ^ 2 + x__2 ^ 2) ^ 2 0; 0 0 0;];
h2_dot = zeta*dot(dh2, gamma)  -  -3;
...
ubar = inv(A)*(- b - diag([k21, k31])*h -  diag([k22, k32])*h_dt);
...
phi__2_ddot = u*dot(dh1,gamma)...
    + om*zeta*dot(dh1,SSS(n)*gamma)...
    + zeta^2*(gamma.'*ddh1*gamma - dot(dh1,gamma.'*N*gamma*n));

phi__3_ddot = u*dot(dh2,gamma)...
    + om*zeta*dot(dh2,SSS(n)*gamma)...
    + zeta^2*(gamma.'*ddh2*gamma - dot(dh2,gamma.'*N*gamma*n));
