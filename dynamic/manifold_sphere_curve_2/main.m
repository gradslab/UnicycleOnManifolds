...
r = 2;
%% Initial conditions
x01 = -0.5; x02 = -0.7; x03 = sqrt(1 - x01^2 - x02^2);
...
%% Fetching data
for i = 1 :length(T)
    [~,h1(i,1),h1_dot(i,1),h2(i,1),h2_dot(i,1), const_1(i,1),const_2(i,1),ch(i,1),ch_dt(i,1)] = geo_unicycle_ext(T(i),Z(i,:),r);
end
%% Plot
...
plot(T,h1); plot(T,h1_dot); plot(T,h2_dot  + 1);
xlabel('Time (s)')
legend('$h_1$','$\dot{h}_1$','$\dot{h}_2$')
...
%% animate
 ...
 % animate([Z(:,1).'; Z(:,2).'; Z(:,3).'], T, r, 'Record', true, 'Filename', 'unicyle_on_manifold.mp4')
snapshots([Z(:,1).'; Z(:,2).'; Z(:,3).'], T, r, [0 1 3  6  7  9 11 12])